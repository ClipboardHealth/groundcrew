...
- **tmux workspace** — by default a window inside a shared `groundcrew` tmux session; set `GROUNDCREW_TMUX_SESSION_PER_TASK=1` in the `crew` launch environment for a dedicated session per task (tagged `@groundcrew_managed`) so each task isolates its own windows/panes.
...
Lifecycle lives in `src/lib/boardSource.ts`. Callers ask `boardSource.verify()` once at startup (fail-fast on a missing project) and `boardSource.fetch()` per tick; nothing else in the package reaches Linear's GraphQL API. The module owns label-based agent parsing (`agent-*` labels) and description-based repository parsing — callers consume a typed `Issue[]`.
