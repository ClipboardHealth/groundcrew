...
export interface TicketSource {
  ...
  /**
   * Optional: live-refresh blockers for an issue. Used by ticketDoctor's
   * per-ticket probe when the snapshot may be stale (e.g., a blocker
   * transitioned to "done" between ticks). If omitted, Board returns
   * issue.blockers from the latest fetch.
   */
  refreshBlockers?(issue: Issue): Promise<Blocker[]>;

  /**
   * Optional: count this source's currently-in-progress issues globally.
   * Used for session-usage gating. If omitted, Board falls back to a full
   * `fetch()` and counts `"in-progress"` issues in memory. The fallback
   * returns the same number as a native implementation would; it's just
   * slower (downloads every issue instead of asking the source for a
   * direct count).
   */
