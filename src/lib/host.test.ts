...
describe(detectHostCapabilities, () => {
  ...
  it("reports each supported host binary as present when all are on PATH", async () => {
    mockWhich(["safehouse", "sbx", "cmux", "tmux", "zellij"]);
    ...
    expect(actual.hasZellij).toBe(true);
  });
  ...
  it("reports every probed binary as missing when which throws", async () => {
    ...
    expect(actual.hasZellij).toBe(false);
  });
