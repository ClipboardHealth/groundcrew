...
5. **Prepare the runner and agent auth.** Groundcrew uses a `cmux` or `tmux` workspace hosting the resolved local backend (`safehouse`, `sdx`, or `none`) plus locally authenticated agent CLIs. Setup fails fast when the resolved backend's binary or platform requirement is missing — `safehouse` requires macOS + `safehouse` on PATH; `sdx` requires macOS or Linux + `sbx` on PATH. `models.isolation` and per-model `isolation` are legacy keys and fail config validation. Per-model `sandbox` blocks are accepted again and used by the `sdx` runner.
   ...
   Opt-in SSH agent forwarding: set `sandbox.forwardSshAgent: true` on a model to bind-mount `dirname($SSH_AUTH_SOCK)` into the sandbox at `sbx create` time and pass `-e SSH_AUTH_SOCK` on every `sbx exec`. The agent inside the sandbox can then `git push` via the host's SSH agent (e.g. 1Password's). The socket directory is fixed at create time, so if your `$SSH_AUTH_SOCK` path is ephemeral (changes per shell session), recreate the sandbox after it drifts (`sbx rm groundcrew-<repo>-<model>` and rerun). When `forwardSshAgent` is true but `$SSH_AUTH_SOCK` is unset at run time, groundcrew logs a warning and skips the forward — the launch still proceeds.
...
| `models.definitions.<name>.sandbox`     | optional            | Docker Sandboxes binding for the model. Required at launch when `local.runner` resolves to `sdx`. Fields: `agent` (required sbx agent name), `template`, `kits`, `setupCommand` (override for the inside-sandbox setup script), `forwardSshAgent` (opt-in SSH-agent forwarding from the host — see Quickstart step 5). |
