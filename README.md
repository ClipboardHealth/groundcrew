...
Three settings are required; everything else has a default. The Linear source is exactly one of `linear.projects[]` or `linear.views[]` — see [Linear view mode](#linear-view-mode) for the view-mode alternative.

| Key                                                          | What                                                                                                                             |
| ------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------- |
| `linear.projects[].projectSlug` OR `linear.views[].viewSlug` | Trailing slug of each Linear project URL (e.g. `ai-strategy-5152195762f3`) or Linear view URL. Configure exactly one of the two. |
...
`sources` declares extra ticket-system adapters. The current release verifies configured extra sources during `crew run` startup; the dispatch loop still reads Linear through the configured Linear mode (`linear.projects` or `linear.views`) until the consumer refactor lands. This lets you validate shell/Jira/local-plan integrations without changing existing Linear behavior.
...
| --------------------------------------- | ------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `linear.projects`                       | required (or `linear.views`)    | Non-empty array of Linear projects to watch. Mutually exclusive with `linear.views`; configure exactly one. One `crew` instance dispatches across every entry under a shared `maximumInProgress` budget.                                                                                                                                                                |
...
| `linear.views`                          | required (or `linear.projects`) | Non-empty array of Linear views (saved filters). Mutually exclusive with `linear.projects`. Canonical status mapping comes from workflow state types — see [Linear view mode](#linear-view-mode).                                                                                                                                                                       |
| `linear.views[].viewSlug`               | **required**                    | Linear view URL slug (e.g. `george-bezerra-tasks-61e51e3730dd`). The trailing 12-char hex `slugId` is matched against Linear's API.                                                                                                                                                                                                                                     |
