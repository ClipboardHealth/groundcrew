...
| `agents.definitions.<name>.preLaunchEnv`              | optional             | Companion to `preLaunch`: list of env var names to append to groundcrew's Safehouse `--env-pass=` flag, so `preLaunch` exports reach the agent without overriding `cmd`. Each entry must match `[A-Za-z_][A-Za-z0-9_]*`. `none` clears those names before `preLaunch`; its exports then inherit as-is. An empty array is a uniform no-op under every runner; non-empty lists reject `safehouse` commands, `sdx` runners, or Groundcrew-managed worker metadata names. |
