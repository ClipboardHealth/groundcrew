...
| `local.readOnlyDirs`                                  | `~/.config/tfenv`    | Host directories re-opened read-only inside the Safehouse sandbox, for toolchains the profile masks but does not re-open. `~` is expanded. Defaults to tfenv's config root so `terraform`/`tfenv` work in the sandbox; set your own list to add or replace entries. Under `safehouse`, groundcrew also auto-resolves symlinks under the agent's config directory (`~/.claude`) and grants their targets read-only, so a dotfiles-managed config (stow, chezmoi, yadm, dotbot) works without manual entries; a file link grants the resolved file, a directory link the resolved directory, and a link resolving to `$HOME` or `/` is refused with a log line — add such a path here explicitly if you want it. Ignored by `sdx` and `none`. |
