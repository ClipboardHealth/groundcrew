...
| `git.stacking`                                        | `false`              | When `true`, a Todo task blocked by exactly one unresolved same-repo blocker is dispatched as a child stacked on the blocker's branch instead of waiting for the blocker to merge. Only a literal `true` opts in. Config is cached per process, so a running `crew run --watch` needs a restart to pick up a change. Add the `groundcrew-no-stack` Linear label to an individual task to opt it out.                                                                                                             |
