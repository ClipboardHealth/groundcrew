...
| `git.stacking`                                        | `false`              | When `true`, a Todo task blocked only by unresolved same-repo blockers that form a single chain (one blocker, or several where one blocker's stack ancestry covers the rest) is dispatched as a child stacked on the chain tip's branch instead of waiting for the blockers to merge. Only a literal `true` opts in. Config is cached per process, so a running `crew run --watch` needs a restart to pick up a change. Add the `groundcrew-no-stack` Linear label to an individual task to opt it out.          |
