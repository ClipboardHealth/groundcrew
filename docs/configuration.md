...
- every file git itself reports loading, via `git config --list --show-origin` run from the worktree. git is asked rather than walked because `~/.gitconfig` is commonly a _real_ file whose indirection is an `include.path` directive, which no symlink resolution can find; asking git also picks up `includeIf` conditional includes for free. A git config file it cannot read is fatal to every git command in the sandbox, so this one is worth not guessing at.
