...
Groundcrew ships built-in presets for `claude`, `codex`, and `composer`, but agents are not enabled by default. List the agents you want in `agents.definitions`:
...
`crew resume` reopens the agent's previous conversation in the worktree — **no config required** for the built-in agents. The shipped `claude`, `codex`, and `composer` presets carry a `resumeArgs` default (`--continue`, `resume --last`, and `--continue`), which groundcrew appends to the agent's command on resume.
...
| `agents.definitions`                                  | **required**         | Enabled launch profile set. Built-in keys (`claude`, `codex`, `composer`) can use `{}` to opt into the shipped preset. Custom profile names must provide `cmd` and `color`; use custom profiles such as `claude-fable` and `claude-opus` to select model-specific commands per task.                                                                                                                                                                                  |
...
| `agents.definitions.<name>.resumeArgs`                | preset default       | Shell args appended to `cmd` on `crew resume` so the agent [reopens its previous conversation](#resuming-the-agents-conversation) in the worktree. Defaults to `"--continue"` (claude, composer) and `"resume --last"` (codex) on the built-in presets; set it for custom agents or to override. `crew resume --new` ignores it and cold-starts. No session id is stored.                                                                                             |
