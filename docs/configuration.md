...
| `workspace.useTaskTitleForPanelName` | Opt-in: title cmux panels with the ticket title instead of the task id. |
...
## Dotfiles-managed config (auto-resolved symlinks)

When a dotfiles manager (stow, chezmoi, yadm, dotbot) owns your config, entries like `~/.claude/settings.json` are symlinks into a repo elsewhere on disk. macOS seatbelt resolves symlinks **before** evaluating policy, so a grant of `~/.claude` leaves the kernel checking the ungranted target and the agent sees `EPERM: operation not permitted, stat '/Users/<user>/.claude/settings.json'` for a file that plainly exists outside the sandbox. Claude Code then skips the settings file entirely; the same failure hits `skills`, `hooks`, `agents`, `commands`, `CLAUDE.md`, and `mcp.json`. No safehouse flag fixes this — seatbelt has no "trust the link" bit — so under `local.runner: "safehouse"` groundcrew resolves the links at launch and appends their targets to the read-only grant list.

What is resolved:

- the agent's own config directory (`~/.claude`, or `~/.pi/agent` for Pi; codex is excluded because its `CODEX_HOME` is relocated to a staged writable home instead of read in place)
- the host tool configs every agent shells out to: `~/.config/gh` and `~/.config/git`
- every file git itself reports loading at **system and global scope**, via `git config --list --show-scope --show-origin` run from the worktree. git is asked rather than walked because `~/.gitconfig` is commonly a _real_ file whose indirection is an `include.path` directive, which no symlink resolution can find; asking git also picks up `includeIf` conditional includes for free. A git config file it cannot read is fatal to every git command in the sandbox, so this one is worth not guessing at. Repository-local and per-worktree scopes are dropped: `.git/config` is writable by the sandboxed agent and by repo-controlled hooks, and an `include.path` there names any file on the host, so honoring it would let a repo widen its own sandbox on the next launch. Local config lives inside the already-granted worktree, so nothing is lost.

That middle scope is deliberate. Resolving only the agent's config directory is too narrow — it leaves `gh` unable to start, so an agent cannot open the PR it was dispatched to open. Resolving all of `~/.config` is too wide: a typical dotfiles setup symlinks a dozen unrelated entries (`nvim`, `ranger`, `raycast`, `opencode`, …), and following all of them is a large inference to make silently. The allowlist covers exactly the tools groundcrew's own workflow depends on; anything else stays an explicit `local.readOnlyDirs` line.
...
- Auto-resolution is skipped entirely when the agent's `cmd` starts with `safehouse` — that cmd owns its own wrap and receives none of groundcrew's flags, so resolving would announce grants the sandbox never gets.
- Auto-grants are capped at 256 paths, and the cap is announced when it trips — the list scales with the size of your dotfiles repo, not with anything groundcrew bounds. A path already inside a granted directory is dropped rather than granted twice. Directories named `cache` or `node_modules` are never descended into; `~/.claude/plugins/cache` holds whole cached repositories.
- A one-line count of the auto-grants is logged at normal level; the individual paths are logged at `--verbose`. A silently widened sandbox would defeat the point of the guard below.
...
- A link into a credential store (`~/.ssh`, `~/.aws`, `~/.gnupg`, `~/.kube`, `~/.docker`, `~/.config/gcloud`, `~/.config/op`, `~/Library/Keychains`) is **refused**, as is any path containing `:` — safehouse joins the grant list on that character, so such a path would be delivered as two. The sandboxed agent can write its own config directory, so it could otherwise plant a link to a credential file and have the next launch grant it.
- A link resolving to `$HOME`, the directory holding every home (`/Users`, `/home`), or `/` is **refused** and logged, naming the path. A read-only grant of your home directory would expose `~/.ssh`, `~/.aws/credentials`, browser cookies, and every other repo to an agent with network egress. Write `local: { readOnlyDirs: ["~"] }` if you truly want it — the guard does not forbid it, it requires saying so out loud in a file that shows up in review.
...
Explicit `local.readOnlyDirs` entries always still apply. Ignored by the `sdx` and `none` runners.
...
| `local.readOnlyDirs`                                  | `~/.config/tfenv`    | Host directories re-opened read-only inside the Safehouse sandbox, for toolchains the profile masks but does not re-open. `~` is expanded. Defaults to tfenv's config root so `terraform`/`tfenv` work in the sandbox; set your own list to add or replace entries. Under `safehouse`, groundcrew also auto-resolves dotfiles-managed symlinks and grants their targets read-only — see [Dotfiles-managed config (auto-resolved symlinks)](#dotfiles-managed-config-auto-resolved-symlinks). Ignored by `sdx` and `none`. |
