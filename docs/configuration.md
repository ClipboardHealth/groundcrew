...
Paths are worktree-root-relative and matched literally. Anything undeclared
still blocks teardown, so real agent work is never discarded silently. See
[`hookGeneratedPaths`](./setup-hooks.md#hookgeneratedpaths-when-the-hook-dirties-the-worktree)
for the full contract and its constraints.
...
| `workspace.knownRepositories[].hookGeneratedPaths`    | optional             | Worktree-root-relative paths the `prepareWorktree` hook is expected to create or rewrite, excluded from the dirtiness guard so teardown stops mistaking hook output for the agent's uncommitted work. Operator-only: a repo-committed `.groundcrew/config.json` that sets it is a hard config error. Replaces `defaults.hookGeneratedPaths` rather than merging with it. Matched literally — absolute paths, `.`/`..` segments, and leading-colon pathspec magic are rejected at load. Declared paths are force-discarded on teardown, so list files, not broad parent directories. |
| `defaults.hookGeneratedPaths`                         | optional             | Global fallback for `workspace.knownRepositories[].hookGeneratedPaths`, used only by repositories that do not declare their own list.                                                                                                                                                                                                                                                                                                                                                                                                                                               |
