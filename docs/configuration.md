...
| `workspaceKind`                          | `"auto"`             | Terminal session manager. `"auto"` picks `cmux` when on PATH, else `tmux`. Set to `"cmux"`, `"tmux"`, or `"zellij"` to fail loudly when the chosen backend is missing. `"zellij"` (Linux/WSL) runs one named tab per ticket in a shared `groundcrew` session with native mouse support; attach with `zellij attach groundcrew`.                                                                                                                                                                                             |
