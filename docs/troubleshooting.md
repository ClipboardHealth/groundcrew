...
If `codex: {}` is listed, doctor expects the `codex` CLI to be installed because tasks can route to `agent-codex` and `agent-any` can select it. Likewise, `cursor: {}` expects the `cursor-agent` CLI — the preset's command is `cursor-agent`, not `cursor` (`curl https://cursor.com/install -fsS | bash` installs it standalone).

## Safehouse-Wrapped Commands Are Not Re-Wrapped

If an `agents.definitions.<name>.cmd` already starts with `safehouse`, groundcrew assumes that command owns its Safehouse flags and does not add the `safehouse-clearance` wrapper a second time. Changing the proxy's allowlist after it is running requires killing the PID in `${XDG_CACHE_HOME:-$HOME/.cache}/clearance/clearance.pid` so the next launch picks up the new env.
...
## Cursor Runs With Its Sandbox Disabled

The shipped `cursor` command is `cursor-agent --sandbox disabled --force`. Groundcrew already runs the agent under the resolved `local.runner`, and cursor's built-in sandbox would nest inside it and fail — the same reason the `codex` preset passes `--dangerously-bypass-approvals-and-sandbox`. `--force` is cursor's auto-approve mode, matching claude's `--permission-mode auto`. Override `agents.definitions.cursor.cmd` for a stricter mode.
