...
- **Built-in credential profiles**: the per-agent read/write/deny profiles above ship for `claude` and `codex`. The `composer` preset does **not** yet include an `srt` profile, so under `srt` its `~/.cursor` credentials stay behind the home read-mask and `cursor-agent` cannot authenticate. Run composer under `safehouse` or `none` until a validated profile is added, or re-open its config dir yourself via `local.readOnlyDirs` (read-only auth may still not suffice, since `cursor-agent` writes chat state under `~/.cursor`).
