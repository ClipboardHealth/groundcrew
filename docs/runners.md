...
- **Built-in credential profiles**: the per-agent read/write/deny profiles above ship for `claude` and `codex`. The `cursor` and `cursor-grok` presets do **not** yet include an `srt` profile, so under `srt` their `~/.cursor` credentials stay behind the home read-mask and `cursor-agent` cannot authenticate. Run them under `safehouse` or `none` until a validated profile is added, or re-open the config dir yourself via `local.readOnlyDirs` (read-only auth may still not suffice, since `cursor-agent` writes chat state under `~/.cursor`).
